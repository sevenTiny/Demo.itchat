...
def text_reply(msg):
    ...
    print('+ '+str(datetime.datetime.now)+' 收到：' +
          msg['User']['NickName']+'发来的消息。消息内容为：'+msg['Content'])

    # 这里最后写自己所在的群号
    if msg['User']['NickName'] == 'temp':
        ...
        if match:
            ...
            itchat.send('%s\n%s' %
                        (username, REPLY['工作'][randomIdx]), msg['FromUserName'])
        ...
        if match:
            ...
            itchat.send('%s\n%s' %
                        (username, REPLY['学习'][randomIdx]), msg['FromUserName'])
        ...
        itchat.send('%s\n%s' % (
            username, REPLY['default'][randomIdx]), msg['FromUserName'])
        print('-+-+'*5)
